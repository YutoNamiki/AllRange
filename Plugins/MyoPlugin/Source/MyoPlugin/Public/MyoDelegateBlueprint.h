...
public:
	...
	virtual void OnConnect(int32 myoId, uint64 timestamp) override;
	virtual void OnDisconnect(int32 myoId, uint64 timestamp) override;
	virtual void OnPair(int32 myoId, uint64 timestamp) override;
	virtual void OnUnpair(int32 myoId, uint64 timestamp) override;
	virtual void OnArmMoved(int32 myoId, FVector armAcceleration, FRotator armOrientation, FVector armGyro, MyoPose pose);
	virtual void OnOrientationData(int32 myoId, uint64 timestamp, FRotator rot) override;
	virtual void OnAccelerometerData(int32 myoId, uint64 timestamp, FVector accel) override;
	virtual void OnGyroscopeData(int32 myoId, uint64 timestamp, FVector gyro) override;
	virtual void OnPose(int32 myoId, uint64 timestamp, MyoPose pose) override;
	virtual void OnArmSync(int32 myoId, uint64 timestamp, MyoArm arm, MyoArmDirection direction) override;
	virtual void OnArmUnsync(int32 myoId, uint64 timestamp) override;
